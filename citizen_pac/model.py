...
def generateFoodGrid(width, height):
    ...
    # width[0..a] has food in all x positions
    a = 0
    while a < nx:
        new_food_x = a*dx + tx
        a = a + 1
        # height[0..d] has food in all y positions
        d = 0
        while d < ny:           
            new_food_y = d*dy + ty
            d = d + 1
            new_tuple = (new_food_x, new_food_y, randomColor())
            all_food.append(new_tuple)
            
    # grid has food in all x positions width[0..len(width)-1]
    # and in all y positions height[0..len(height)-1]
    return all_food
...
class Scene(QtGui.QGraphicsScene):
    ...
    def reset(self):
        ...
        for element in self.ghosts:
            element.reset()
        for element in self.food:
            element.reset()
